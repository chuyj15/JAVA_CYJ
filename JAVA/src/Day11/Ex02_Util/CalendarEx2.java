...
public class CalendarEx2 {
	public static void main(String[] args) {
		...
		System.out.println(cal.get(Calendar.MONTH) +1 );		//반드시 +1을 해줘야 하네
		...
		//이번 주 로또 추첨일시
		//매일 6시부터 24시까지 판매
		//추첨일(토요일)에는 오후8시에 판매 마감
		//추첨일 오후 8시부터 다음날(일요일) 오전 6시까지는 판매 정지
		
		//조건
		//일요일 오전 6시~ 토요일 오후 8시에 구매 : 이번주 토요일 오후 9시
		...
		//1. 토요일이면서, 오후8시이후 - 구입 불가
		else if (todayOfWeek.equals("토") && ampm ==1 && 시간>=8){
			System.out.println("구입불가");
		}
		//2. 일요일이면서, 오전6시이전 - 구입 불가
		else if (todayOfWeek.equals("일") && ampm ==0 && 시간>=6){
			System.out.println("구입불가");
		}
		//3. 일요일이면서, 오전6시이후 - 다음주 토요일 9시가 추첨일시 [+6일 후]
		//4. 그 외, 이번주 토요일 오후9시가 추첨일시 [+(7-요일)]
		else {
