...
public class StringEx {
	public static void main(String[] args) {
		...
		System.out.println(str1.equals(str2)); 			//문자열 일치 여부 확인(true/false)		//false
		System.out.println(str1.equalsIgnoreCase(str2)); //대소문자 구분없이 문자열 일치 여부 확인(true/false)		//true
		System.out.println(str1.indexOf("A")); 			//해당 문자의 첫 index 반환, 없으면 -1		//10
		System.out.println(str1.lastIndexOf("e")); 		//해당 문자의 마지막 index 반환, 없으면 01		//14
		...
		//split("구분자")				: 구분자를 기준으로 문자열을 잘라서 배열로 반환
		...
		String cal = "10+20";
		String[] num = cal.split("\\+");		//이 때 +기호를 바로 쓰면.. +기호안에는 여러 의미가 있어서.. 정규표현식말고 써야 함. 
		//그래서 +만 쓰면 에러가 남. 그래서 백슬래시 두 개를 넣어야 함. 
		//정규표현식의 + 기호가 아닌 산술연산자 +로 구분하려면, 앞에 \\를 붙여주어야 한다. 
