...
//			int i=0;
			...
			while( rs.next() ) {		// next() : 실행 결과의 다음 데이터로 이동
				...
				//결과 데이터 가져오기
				//rs.getXXX("칼럼명") --> 해당 칼럼의 데이터를 가져온다. / XXX타입으로.
...
//				if (i>=max) break;
	...
	public Text select(int no) {
		...
		try {
			...
			// setXXX(순서, 값) : SQL의 지정한 순서(?)에 있는 파라미터에 값을 매핑
			// - 여기서는, "1번 ?에 글번호 no값을 매핑한다."
			rs = psmt.executeQuery();			//쿼리 실행
			//executeQuery();
			 // : SQL( SELECT)을 실행하고 결과를 ResultSet 객체로 반환
			
			//조회결과 가져오기
			if ( rs.next() ) {	//사실 한 건만 있겠죠
				board.setBoardNo( rs.getInt("board_no") );
				board.setTitle( rs.getString("title") );
				board.setWriter( rs.getString("writer") );
				board.setContent( rs.getString("content") );
	...
	public int insert(Text text) {
		...
		String sql = " INSERT INTO board( title, writer, content )"
					+ " VALUES( ?, ?, ? ) ";
		try {
			psmt = con.prepareStatement(sql);     //쿼리 실행 객체 생성 (repareStatement)
