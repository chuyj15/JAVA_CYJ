...
public class Ex07_File {
	...
	public static File[] listDirectory(String filePath) {
		...
		for (File subFile : fileList) {
			...
			Date date = new Date(update);	//long타입 매개변수를 Date클래스에 넣어줌. 
			SimpleDateFormat sdf = new SimpleDateFormat("yyyy/MM/dd HH:mm:ss");	//포맷을 지정해서 객체 생성
			String updateDate = sdf.format(date);
			
			System.out.println(subFile.getName() + "\t\t");		//파일명
			System.out.println(subFile.length() + "\t\t");		//파일크기	//바이트단위의 용량
	...
	public static void fileInfo(String filePath) {
		...
		System.out.println(file.length() + "\t\t");		//파일크기	//바이트단위의 용량
		System.out.println(updateDate + "\t\t");		//수정일자
		System.out.println();
	}
	...
	public static void deleteFile(String filePath) {
		...
		if (file.exists()) {
			...
			// 폴더를 삭제하려고 할 때 그 하위에 파일이 존재하면 삭제가 안됨.
			if (file.delete()) {
				System.out.println("파일 삭제 성공!!");
			} else {
				System.err.println("파일 삭제 실패!!"); 		//// 폴더를 삭제하려고 할 때 그 하위에 파일이 존재하면 삭제가 안됨.
			}
		} else {
